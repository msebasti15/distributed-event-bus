...
- Unique message IDs generated by clients.
- Explicit idempotency keys, independent from message identity.
- Binary publish acknowledgements with published, duplicate, and rejected
  statuses.
- Bounded broker-side deduplication by topic and message ID.
- Connection Manager retries using the same message ID after a failed write.
- Consumer delivery acknowledgements and bounded duplicate filtering.
- Bounded delivery retries with documented timeout and failure boundaries.
- Configurable broker logging levels with verbose delivery tracing.
